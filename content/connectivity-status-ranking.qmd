...
data <- data %>%
  rename(
    `Barrier ID` = aggregated_crossings_id,
    `Site Name` = internal_name,
    `Watershed Name` = watershed_group_code,
    `Structure Status` = structure_list_status,
    `Rank Order` = rank_combined,
    `Average Habitat Gain (km)` = all_spawningrearing_belowupstrbarriers_km
  )
ft <- flextable(data)
ft <- format_flextable(ft)
ft
