...
Models were run to produce initial connectivity outputs, and maps were created showing disconnected habitat and associated structures. A “longest fragment” approach was used, whereby connectivity status was estimated by calculating the proportion of habitat found in the single largest interconnected area (i.e., not fragmented by human-made barriers). Habitat separated from this area by one or more barriers or unassessed structures was considered disconnected.  


## Streams Permanently Altered by Mining {-}

A number of mines exist in Qukin ?amak?is, primarily for coal and coal-related products. Mining practices often result in large-scale impacts to the landscape, some of which are irreversible in a short period of time. Many small, first order streams and a small handful of larger streams are affected by the mining process to an extent that the streams are no longer accessible for use by aquatic species. For the purposes of this WCRP, streams permanently altered by mining are classified as those streams that no longer exist due to mining practices. These areas were delineated by overlaying mapped mineral tenures (permitted areas and notices of work) in Qukin ?amak?is from the BC Geographic Warehouse and overlaying it on ortho imagery to determine where streams have been lost or permanently altered. A measure of the number of kilometres of habitat from the Habitat Model that overlaps with the permanently altered by mining layer was then calculated for those areas both upstream and downstream of Elko Dam.  
...
To manage this, both the overall habitat upstream, and immediate gains (i.e., the amount of habitat to the next structure) are incorporated into the ranking process. This ensures that the first structure encountered by fish is given a high rank, even if, for example, it only blocks less than 1 km of habitat. Conversely, a structure blocking 20 km of habitat may be given a lower rank if many structures must be addressed before those gains can be realized.  
