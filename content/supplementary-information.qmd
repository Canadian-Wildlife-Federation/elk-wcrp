...
Effectiveness evaluation of identified conservation strategies and associated actions to improve connectivity for Westslope Cutthroat Trout in the Elk River watershed. The planning team identified two broad strategies to implement through this WCRP, 1) barrier rehabilitation and 2) barrier prevention. Individual actions were qualitatively evaluated based on the anticipated effect each action will have on realizing on-the-ground gains in connectivity. Effectiveness ratings are based on a combination of "Feasibility" and "Impact", Feasibility is defined as the degree to which the project team can implement the action within realistic constraints (financial, time, ethical, etc.) and Impact is the degree to which the action is likely to contribute to achieving one or more of the goals established in this plan.
...
data = pd.read_csv('data/strategy-barrier-remediation.csv', index_col=False, skip_blank_lines=False )
...
def highlighttab7(val):
    ...
    elif val =="Low" or val=="Not Effective": color = red
    else: color = 'white'
...
data = pd.read_csv('data/strategy-barrier-prevention.csv', escapechar='\n', index_col=False)
...
![Theory of change developed by the planning team for the actions identified under Strategy 1: Barrier Rehabilitation in the Elk River watershed.](images/flowchart-barrier-rem.png){#fig-stra1}
...
## Funding Sources {-}
```{r fund1, echo = FALSE, results = 'asis'}
#| label: tbl-fund1
#| tbl-cap: "Confirmed funding sources for plan implementation in the Elk River watershed."
#| warning: false

data <- read.csv("data/confirmed-funding-sources.csv", check.names=FALSE)
ft <- flextable(data)
ft <- format_flextable(ft)
ft
```

```{r fund2, echo = FALSE, results = 'asis'}
#| label: tbl-fund2
#| tbl-cap: "Potential funding sources for plan implementation in the Elk River watershed. The Canadian Wildlife Federation and the planning team can coordinate proposal submission through these sources."
#| warning: false

data <- read.csv("data/potential-funding-sources.csv", check.names=FALSE)
ft <- flextable(data)
ft <- format_flextable(ft)
ft
```
